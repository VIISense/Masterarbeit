...
\If {$\exists q, 1\leq q\leq p:\, s^\prime_q$ appears syntactically with the same sign in $t$}
...
\item\label{exceeded}$merge$-rule: $S$ contains $x:succ(c)$, with $c\in\{k\leq l,\,k<l\}$, which is violated, $(x,y_1):s_1\in S$ and $(x,y_2):s_2\in S$, with $s_1$ and $s_2$ in $k$ and $y_1$ and $y_2$ being unblocked by any assertions\\
