...
\end{mydef}
For the Tableau-algorithm in this work a merging can only occur if an assertion $x:succ(k\lesseqgtr l)$ is violated.\\
...
\item\label{leq}$\leq$-rule: $S$ contains $x:succ(c)$, with $c\in\{k\leq l,\,k<l\}$, which is violated, and there is a safe set term $s:=|s_1\cap \dots \cap s_j|$ regarding $c$:\\
$\rightarrow$ $S:=S\cup\{(x,y):s\}$. Then jump to rule \ref{repeat}
\item\label{exceeded}$merge$-rule: $S$ contains
\begin{itemize}
\item $x:succ(c)$, with $c\in\{k\leq l,\,k<l\}$, which is violated
