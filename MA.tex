...
\item we apply the $succ$-rule on $x:succ(c)$: By the induction hypothesis we have $x^\mathcal{I}\in succ(c)^\mathcal{I}$. We have to show that by this step we are able to add successors, such that $\mathcal{I}$ satisfies $\mathcal{A}_{i+1}$. In this step, we gather first all $succ$-assertion together, formulate a $QFBAPA$ formula and let a solver return us all possible \textcolor{red}{(change it later to some upper bounds)}. Because $\mathcal{A}_i$ is satisfiable, the $succ$-assertions together are also satisfiable (subset of $\mathcal{A}$). Hence there has to solutions which can be returned by the solver. By the induction hypothesis we know that $x^\mathcal{I}\in succ(c)^\mathcal{I}$. In case $x$ has no successor, the solution $\phi$ with $E(\phi)=\emptyset$ must be a valid solution, which can be returned from the solver. In the case where $x$ has successors in $\mathcal{I}$, we can create a solution $\phi^\ast$ base on that: For each element $y^\mathcal{I}$ such that $(x^\mathcal{I},y^\mathcal{I}):r^\mathcal{I}$, $r\in\mathbf{R}$, we create an element $e$ and add it to $E(\phi^\ast)$
\begin{itemize}
\item for each $(x^\mathcal{I},y^\mathcal{I}):r^\mathcal{I}$, $r\in\mathbf{R}$, add $e$ to $X_r$
\item for each $y^\mathcal{I}\in C^\mathcal{I}$, $C\in\mathbf{C}$, add $e$ to $X_C$
\item for each $y^\mathcal{I}\in succ(c)^\mathcal{I}$, add $e$ to $X_{c}$
\end{itemize}
Note that $\mathcal{I}$ can be infinite. However it is enough if the solver returns a solution $\phi$, such that $\phi$ can be extended to $\phi^\ast$.\\
