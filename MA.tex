...
\item we apply the $succ$-rule on $x:succ(c)$: By the induction hypothesis we have $x^\mathcal{I}\in succ(c)^\mathcal{I}$. We have to show that by this step we are able to add successors, such that $\mathcal{I}$ satisfies $\mathcal{A}_{i+1}$. In this step, we gather first all $succ$-assertion together, formulate a $QFBAPA$ formula $\mathcal{F}(x)$ and let a solver return us all possible solution with in an upper bound. Because $\mathcal{A}_i$ is satisfiable, the $succ$-assertions together are also satisfiable (subset of $\mathcal{A}$). Hence there has to solutions which can be returned by the solver. By the induction hypothesis we know that $x^\mathcal{I}\in succ(c)^\mathcal{I}$. In case $x^\mathcal{I}$ has no successors, the solution $\phi$ with $E(\phi)=\emptyset$ must be a valid solution, which can be returned from the solver. In case $x^\mathcal{I}$ does have successors, we can create a finite solution $\phi^\ast$ within an upper bound from $\mathcal{I}$:
\begin{algorithm}[H] 
\begin{algorithmic}[1]
\State $\phi^\ast=\emptyset$
\State $E(\phi^\ast)=\emptyset$
\State $u\in\mathbb{N}$ upper bound
\While{$\phi$ is not a solution of $\mathcal{F}(x)$}
\State pick one successor $y^\mathcal{I}$, which has not been picked yet
\State create $\phi_{dummy}$
\State introduce $e$ and add it to $E(\phi_{dummy})$
\ForEach{$(x^\mathcal{I},y^\mathcal{I})\in r^\mathcal{I}$}
\If{$|r^\mathcal{I}(x)|=u$}
\State jump to line 5;
\Else
\State add $\phi(e)=X_r$ to $\phi_{dummy}$
\EndIf
\EndFor
\ForEach{$y^\mathcal{I}\in C^\mathcal{I}$, $C\in\mathbf{C}$}
\If{$|\bigcup_{r\in\mathbf{R}}r^\mathcal{I}(x)\cap C^\mathcal{I}|=u$}
\State jump to line 5;
\Else
\State add $e$ to $X_C$
\EndIf
\EndFor
\ForEach{$y^\mathcal{I}\in succ(c)^\mathcal{I}$}
\If{$|\bigcup_{r\in\mathbf{R}}r^\mathcal{I}(x)\cap succ(c)^\mathcal{I}|=u$}
\State jump to line 5;
\Else
\State add $e$ to $X_c$
\EndIf
\EndFor
\State $\phi^\ast:=\phi^\ast\cup \phi_{dummy}$
\EndWhile
\end{algorithmic}
\end{algorithm}
Since we know that if an ILP has a solution then there must be also a solution with in an upper bound and since $QFBAPA$ formula can be transformed into a ILP, we know that the creation of $\phi^\ast$ must terminates, because $\mathcal{I}$ has a solution.\\
