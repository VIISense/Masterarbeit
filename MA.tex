...
\end{itemize}
\end{mydef}
An ABox $S$ in $\mathcal{ALCSCC}$ is a finite set of assertions of the form $x:C$ and $(x,y):r$, where $C$ is a $\mathcal{ALSCSS}$ concept, $r\in\mathbf{R}$ and $x,y$ are variables. The set $Var(S)$ is the set of variables occurring in $S$. 
...
\begin{align*}
S=\{x:succ(1\leq|succ(|A|\leq|B\cap r|)|), x:succ(|A|\leq |B|), x:C, (x,y):B\cap r\}
\end{align*}
\end{ex}
...
\begin{itemize}
\item for each $(x,y):r$, $r\in\mathbf{R}$:  add $e$ to $X_r$
\item for each $y:C$, $C\in\mathbf{C}$ add $e$ to $X_C$
...
\item\label{set} $set$-rule: $S$ contains $(x,y):s$ and
\begin{itemize}
\item $s=s_1\cup s_2$ and either $(x,y):s_1\notin S$ or $(x,y):s_2\notin S$ $\rightarrow$ $S:=S\cup\{(x,y):s_1,(x,y):s_2\}$
\item $s=s_1\cup s_2$ and neither $(x,y):s_1\in S$ nor $(x,y):s_2\in S$ $\rightarrow$ $S:=S\cup\{(x,y):s_1\}$ or $S:=S\cup\{(x,y):s_2\}$
\item $s=C$, $C$ is a $\mathcal{ALCSCC}$ concept $\rightarrow$ $S:=S\{y:C\}$
\end{itemize}
\end{itemize}
Numerical part:
\begin{itemize}
...
\end{align*}
We are able to apply the boolean rules and hence we apply them first and decompose into Example 1. It is also possible to start with the $set$rule and decompose $(x,y):B\cap r$. When no more rules from the boolean part are applicable we have
\begin{align*}
