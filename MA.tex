...
\end{align*}
\end{ex}
We see that the assertion $x:succ(|A\cup B|\leq 1)$ is violated and a solution is to merge $y_1$ and $y_2$. This leads to $x:succ(|t|+|A\cup B|\geq 4)$ being violated. We can fix it by adding new successors. We can easily detect that adding a $t$-successor leads to a satisfied ABox. On the other hand a successor in $A\cup B$ leads to the initial state where $x:succ(|A\cup B|\leq 1)$ is violated. This can lead us into a endless loop of adding and merging variables. Hence we introduce a notion of \textit{blocking}. If we merged two variable $y_1$ and $y_2$ because of an assertion $x:succ(k\lesseqgtr l)$, $k=n_1\cdot|s_1|+\dots +n_j\cdot |s_j|$, then we want to \textit{block} any introduction of an assertion of the form $z:s_1\cap\dots\cap s_i$, $1\leq i\leq j$. This way we avoid that the assertion $x:succ(k\lesseqgtr l)$ becomes violated again.
\begin{mydef}[Blocking]
Let $x:succ(k\lesseqgtr l)$, $k=n_1\cdot|s_1|+\dots +n_j\cdot |s_j|$, be an assertion which causes the merging of two successors $x$ into one successor $y$. Every set term of the form $s_1\cap\dots\cap s_i$, $1\leq i\leq j$, is \textit{blocked} by $x$.
\end{mydef}
