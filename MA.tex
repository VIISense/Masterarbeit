...
\textbf{Abstract}
\end{center}
In the research field of description logic (DL) checking satisfiability of $\mathcal{ALCQ}$ has been investigated thoroughly and is therefore well-known in this research field. The DL $\mathcal{ALCSCC}$ extend $\mathcal{ALCQ}$ with a numerical aspects in form of cardinality constraints over concepts. There has been some research about checking satisfiability of this DL however creating a Tableau algorithm, which is a method to check satisfiability, for this DL comes with a create challenge due to the numerical aspect. In this work we create a Tableau algorithm with the help of a arbitrary but correct $QFBAPA$ solver. To ensure termination and not using infinite space we only consider solutions which lay sunder a pre-computed upper bound without losing any important information. In the end we provide a full correction proof for this Tableau algorithm.
\tableofcontents
\newpage
...
\end{itemize}
\end{mydef}
Note that in 2.1 we mention that we do only consider cardinality constraints. Further more for the cardinality constraint $c$ can appear only in two forms: $n\,dvd\,l$ and $k\leq l$, with $n\in\mathbb{N}$ and $k,l$ being cardinality terms.\\ 
...
Regarding our example from 2.1 we can now construct an ABox of specific individual names, in which we describe their bodies. Let $\mathbf{C}=\{Legs,Arms,Female\}$ and $\mathbf{R}=\{bodyParts\}$. A possible ABox, which states that an individual name \textit{Anna} has two legs and two arms and is female, is $\{Anna:succ(|Legs\cap bodyParts|=2)\sqcap succ(|Legs|=|Arms|)\sqcap Female\}$.\\
Like for $QFBAPA$ we also need semantics for $\mathcal{ALCSCC}$.
\begin{mydef}[Interpretation of $\mathcal{ALCSCC}$]
...
Considering our ABox with \textit{Anna} we can describe a semnatic like in 2.1: Let $\Delta^\mathcal{I}=\{Anna^\mathcal{I}, Leg1^\mathcal{I}, Leg2^\mathcal{I}, Arm1^\mathcal{I}, Arm2^\mathcal{I}\}$ and 
\begin{itemize}
...
Going back to upper bound problem: The following result from [4] can be used to establish an upper bound for the solution of the \textit{ILP}:
...
In our example the upper bound for all $x_i$ is $8\cdot (2\cdot max_{i,j}\{|1|,|-1|\})^{2\cdot 2+1}=512$, which means that we can discard any solution in which a Venn region has more than $512$ elements.
