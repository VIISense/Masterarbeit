...
\State $\#k:=\#k-n_p$
\EndIf
\If{$(x,y_1):s_p\in S$ and $(x,y_2):s_p\notin S$ $||$ $(x,y_1):s_p\notin S$ and $(x,y_2):s_p\in S$}
\State $\#k:=\#k+n_p$
\EndIf
...
\begin{mydef}[Safe]
Let $x:succ(k\lesseqgtr l)$ be an assertion in $S$ where $k=n_1\cdot|s_1|+\dots+n_i\cdot|s_i|$ and $l=m_1\cdot|t_1|+\dots+m_i\cdot|t_i|$, $i,j\in\mathbb{N}$. Let $u=u_1\cap \dots \cap u_o$ be a set term with $o\leq m$ and $o\leq n$ and each $u$ appears syntactically with the same sign in $l$. The number $n_k(u)$ (and $n_l(u)$ respectively) is computed as followed:
\begin{algorithm}[H] \caption{Compute $n_k(u)$}
\begin{algorithmic}[l]
\State $n_k(u):=0$
\State $k$, cardinality term of the form $n_1\cdot|s_1|+\dots+n_i\cdot|s_i|$
\State $u$, set term of the form $u_1\cap \dots \cap u_o$, $o\leq i$
\ForEach {$n^\prime\cdot|s^\prime_1\cup\dots\cup s^\prime_p|,\,p\in\mathbb{N}$, in $k$}
\If {$\exists q, 1\leq q\leq p:\, s^\prime_q$ appears syntactically with the same sign in $u$}
...
\end{algorithmic}
\end{algorithm}
If $n_k(u)<n_l(u)$ then we call $u$ \textit{safe}.\\
If $k$ is a number e.g. it does not contain a set term, then all $u$ consisting of set terms from $l$ with the same sign are \textit{safe}.
\end{mydef}
