...
\end{itemize}
With $\mathcal{I}(S)$ we can count how many successors a variable has during the Tableau-algorithm. Regarding the cardinality constraints $k\leq l$ and $k<l$ in the assertion $x:succ(k\leq l)$ we have to be \textit{safe} before adding or merging variables. Let $S^\prime$ be the obtained constraint set from $S$:
\begin{itemize}
\item It is safe to introduce a new variable $y$ and add $(x,y):l$ if either
\begin{itemize}
\item $x$ has no successor or
\item $n_{\mathcal{I}(S^\prime)}(x,k,S^\prime)-n_{\mathcal{I}(S^\prime)}(x,k,S)<n_{\mathcal{I}(S^\prime)}(x,l,S^\prime)-n_{\mathcal{I}(S)}(x,l,S)$
\end{itemize}
\item It is safe to merge two variable $y_1,y_2$, for which we have $(x,y_1):s\in S$ and $(x,y_2):s\in S$, in $\mathcal{I}(S)$, if by merging them no assertions regarding $x$ in $S^\prime$ is violated, which was satisfied in $S$
\end{itemize}
...
\end{align*}
\end{mydef}
We want to split $|s_1\cup s_2\cup\dots \cup s_n|$ into set terms such that they are disjoint to each other. This helps to determinate the properties of certain successors. Another positive affect is, that we can omit the $choose$-rule, which are used in \cite{1} and \cite{6}. We still maintain the idea and purpose of this rule because we are force to \textit{choose} a specific property setting for successors.\\
...
Let $x:succ(k\leq l)$ or $x:succ(k<l)$ be violated regarding $x$ in an interpretation $\mathcal{I}$. It is \textit{safe} to introduce a new variable $y$ and add $(x,y):l$ to $S^\prime:=S\cup\{(x,y):l\}$ if 
\begin{align*}
...
\item\label{setterm}$cardinality_1$-rule: $S$ contains $x:succ(c)$, with $c\in\{k\leq l,\,k<l\}$, such that $c$ is violated in $\mathcal{I}(S)$ regarding $x$. Choose either $i:=k$ or $i:=l$. If it is safe to add a variable $y$ in $i$ then\\
$\rightarrow$ $S:=S\cup\{(x,y):i\}$, then jump to rule \ref{repeat}
\item\label{exceeded}$cardinality_2$-rule: $S$ contains $x:succ(n\,dvd\,l)$, which is violated in $\mathcal{I}(S)$ regarding $x$. If we have two successor $y_1\neq y_2$ of $x$ such that for a $|s|$ in $k$ we have $(x,y_1):s\in S$ and $(x,y_2):s\in S$ and by merging them no satisfied constraint regarding $x$ becomes violated, then:\\
...
Another reason shows the next example:
\begin{ex}
\begin{align*}
S=\{x:(succ(|r\cup s| < |r| + |s|)\}
\end{align*}
We first apply rule \ref{setterm} on it and add either $x:r$ or $x:s$. In both cases we have $|r\cup s|=1$. We can apply this rule, because $x$ has no successors yet. Now we can not apply rule \ref{setterm} any more because we already have a successor and by adding either an $r$-successor or $s$-successor the number $|r| + |s|$ does not grow faster than $|r\cup s|$. Hence we need to apply another rule or else the algorithm says that $S$ is not satisfiable. \textcolor{red}{todo}
\end{ex} 
...
\}
\end{align*}
If we try to satisfy all the assertions in the second line with rule \ref{setterm} we end up with at least one assertion being violated: The problem is that by applying rule \ref{setterm} we add one successor $y$ which always raise $|r|$. If we apply \ref{setterm} one time nothing the amount of violated assertions decreases. If we apply it a second time then $x:succ(|r|=1)$ is violated. In case we decided to apply \ref{setterm} on $x:succ(|r|=1)$ and $x:succ(|r\cap s|=1)$ we have the option of either applying rule \ref{setterm} on $x:succ(|r\cap C|=1)$ or applying rule \ref{exceeded} on the two new variables. We decide for the first choice:
\begin{figure}[H]
\centering
\begin{tikzpicture}
\node (0) at (0,0) {$x$};
