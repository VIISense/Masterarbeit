...
\EndIf
\EndFor
\State $\sigma^\ast:=\sigma^\ast\cup \sigma_{dummy}$
\EndWhile
\end{algorithmic}
\end{algorithm}
Since we know that if an ILP has a solution then there must be also a solution with in an upper bound and since $QFBAPA$ formula can be transformed into a ILP, we know that the creation of $\sigma^\ast$ must terminates, because $\mathcal{I}$ has a solution.\\
