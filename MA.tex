...
\item\label{chooserole}$choose$-$a$-$role$-rule: $S$ contains $(x,y):s$ but for no rule name $r$ we have $(x,y):r\notin S$\\
$\rightarrow$ choose one rule name $r\in\mathbf{R}$ which does not occur negated in $s$ and add $(x,y):r$ to $S$
