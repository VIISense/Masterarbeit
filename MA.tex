...
We want to use the Tableau-algorithm to check whether an assertion $x:C$ is satisfiable or not and if it satisfiable we create a satisfied ABox from $x:C$.\\
...
If a $QFBAPA$ solver returns \textit{satisfiable} then calculate the upper bound after Theorem \ref{upper bound} and select one solution $\sigma$ within this bound. For each $e\in E(\sigma)$ we introduce a new variable $y$ and
\begin{itemize}
