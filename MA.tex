...
After updating $b(S,x)$ accordingly we can start the Tableau-algorithm. During the algorithm we still have to update the blocking set $b(S,x)$. Since the Tableau-algorithm does not prefer any rule application it can enter a unless loop of adding and merging successor: We consider the situation where we already have two successors in $A$ and one in $B$: For the first assertion we have $6\leq 5$ and for the second $5\leq 6$. The algorithm then can choose between merging the two successor in $A$ and adding a new successor in $B$. If the algorithm choose merging we do not get nearer to a satisfied ABox. However merging is still a valid rule application \textcolor{red}{todo }
\subsection{Safe}
...
\item there is a set term $s:=|s_1\cap \dots \cap s_i|$, $l=n_1\cdot|s_1|+\dots+n_i\cdot|s_i|+\dots+n_j\cdot|s_j|$, which is safe to introduce
\end{itemize}
