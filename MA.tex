...
The element of $Q$ are multisets of tuples. Each tuple consists of a multiset and an integer. A multiset $M$ is smaller than a multiset $M^\prime$ if we can obtain $M$ from $M^\prime$ by replacing at least one element of $M^\prime$ with at least one element, which is smaller. In our definition of $Q$ $\Psi(\mathcal{A}^\prime)$ is smaller than $\Psi(\mathcal{A})$ if we can replace at least one tuple of $\Psi(\mathcal{A})$ with at least one smaller tuple to obtain $\Psi(\mathcal{A}^\prime)$. A tuple $(x_1,y_1)$ is smaller than a tuple $(x_2,y_2)$ if either $x_1<x_2$ or $x_1=x_2$ and $y_1<y_2$ (lexicographical odering from left to right). Otherwise $(x_2,y_2)$ is smaller.\\
Next we define the size of a concept $C$ inductively, which is always an integer:
\begin{itemize}
...
\end{itemize}
\begin{mydef}
Let $\mathcal{A}$ be a derived ABox. The multiset $\Psi(\mathcal{A})$ consists of tuples. Each tuple $\psi_\mathcal{A}(x)$ represent one variable $x$:
\begin{itemize}
\item the first component is a multiset which contains for each assertion $x:C\sqcap D\in\mathcal{A}$ for which the $\sqcap$-rule is applicable the integer $size(C\sqcap D)$. Respectively for $x:C\sqcup D$
\item the second component is an integer which denotes the number of $x:succ(c)\in\mathcal{A}$ for which the $successor$-rule is applicable
\end{itemize}
\end{mydef}
...
\begin{itemize}
\item $\mathcal{A}^\prime$ is obtained from $\mathcal{A}$ by applying the $\sqcap$-rule: The first component becomes smaller because the integer for $size(C\sqcap D)$ is removed (because we can not apply this rule any more after one application). In case $C$ and/or $D$ happens to be disjunction or conjunction the multiset still becomes smaller because $size(C)\leq size(C)+size(D)=size(C\sqcap D)$ (respectively for $size(D)$). Because the first component always becomes smaller the tuple $\psi_{\mathcal{A}^\prime}(x)$.
\item $\mathcal{A}^\prime$ is obtained from $\mathcal{A}$ by applying the $\sqcup$-rule: similar to above
\item $\mathcal{A}^\prime$ is obtained from $\mathcal{A}$ by applying the $successor$-rule: If we apply this rule, we gather all $succ$-assertion of $x$ and convert them to a $QFBAPA$ formula. If a solver returns unsatisfiable we have add $x:\perp$ which means we end with a clash. If the solver returns a solution, we add successors according to the solution. Hence all $succ$-assertion of $x$ becomes satisfied and the second component decreases. Because we are able to apply this rule, both $\sqcap$-rule and $\sqcup$-rule are not applicable on $\mathcal{A}$. Hence the first component of $\psi_\mathcal{A}(x)$ remains unchanged. Therefore $\psi_{\mathcal{A}^\prime}(x)$ is smaller than $\psi_\mathcal{A}(x)$
\end{itemize}
For any other variables $y$ such that $y\neq x$ the tuple $\psi_\mathcal{A}(y)$ remains unchanged. Hence in all three cases we can obtain $\Psi(\mathcal{A}^\prime)$ from $\Psi(\mathcal{A})$ by replacing $\psi_\mathcal{A}(x)$ with the smaller tuple $\psi_{\mathcal{A}^\prime}(x)$. \\
