...
\end{mylem}
\begin{proof}
Let $\mathcal{I}_{S^\prime}$ be the induced interpretation of the constraint set $S^\prime$ created by the Tableau-algorithm from $S$. We show that there exists an interpretation $\pi_{\mathcal{I}_{S^\prime}}:Var(S^\prime)\shortrightarrow \Delta^{\mathcal{I}_{S^\prime}}$ which satisfies $S^\prime$.\\
We start with the simple assertions $x:C$ and $(x,y):r$ for $C\in\mathbf{C}$ and $r\in\mathbf{R}$ (induction base): By the definition of induced interpretation we assign $\pi_{\mathcal{I}_{S^\prime}}(x):=x^{\mathcal{I}_{S^\prime}}\in C^{\mathcal{I}_{S^\prime}}$. Also by the definition of induced interpretation for every $(x,y):r\in S^\prime$ we have $(\pi_{\mathcal{I}_{S^\prime}}(x),\pi_{\mathcal{I}_{S^\prime}}(y)):=(x^{\mathcal{I}_{S^\prime}},y^{\mathcal{I}_{S^\prime}})\in r^{\mathcal{I}_{S^\prime}}$.\\
Let $S$ be a constraint set and $\pi_{\mathcal{I}_S}$ be an assignment which satisfies $S$ (induction hypothesis).\\
...
If we can apply rule \ref{choose} and obtain $S^\prime$ then we have a constraint $x:succ(k\leq l)$ and a successor $y$ such that $(x,y):k^\prime\in S$, $k^\prime$ occurs in $k$. We then choose between $(x,y):s$ and $(x,y):s^\neg$ for all $|s|$ in $k$ then apply rule \ref{repeat} until this rule is inapplicable. That means at the end we might add several assertions of the form $x:C$ and $(x,y):r$. In case we add $x:C$ we also add $x^{\mathcal{I}_{S^\prime}}$ also to $C^{\mathcal{I}_{S^\prime}}$. Therefore in this case $\pi_{\mathcal{I}_{S^\prime}}$ satisfies $S^\prime$. In case we add $(x,y):r$ we also add $(x^{\mathcal{I}_{S^\prime}},y^{\mathcal{I}_{S^\prime}})$ to $r^{\mathcal{I}_{S^\prime}}$. Hence $\pi_{\mathcal{I}_{S^\prime}}$ satisfies $S^\prime$.\\
If we can apply rule \ref{chooserole} and obtain $S^\prime$ then we have a constraint $(x,y):k$ and but for every role name $r$ we do not have $(x,y):r\in S$.
\end{proof}
\begin{mylem}
