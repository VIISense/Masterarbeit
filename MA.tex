...
A constraint in $NNF$ in a constraint set $S$ is \textit{simplified} if we omit \textit{unnecessary} set terms from any disjunction.\\
...
A constraint is in \textit{simplified} $CNNF$ if we simplified the constraint first and then convert it to $CNNF$. A constrain set $S$ is in simplified $CNNF$ if all constraint are simplified $CNNF$.
\end{mydef}
We consider only simplified $CNNF$ for the algorithm. Also every constraint arise from the algorithm is converted into simplified $CNNF$.
