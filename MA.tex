...
|s^\mathcal{I}\cap\bigcup_{r\in\mathbf{R}}r^\mathcal{I}(x)| & \text{ if } k=|s|
\end{dcases}
\end{align*}
...
The $choose$-$a$-$role$-rule is necessary because for a constraint $x:succ(c)$ we might have no role name with a positive sign in $c$. Which means we know $x$ must have some successors but we can not decide which role-successor it is. As example we have
\begin{ex}
\begin{align*}
&\mathbf{R}=\{r,s\}\\
&S=\{x:succ(|r^\neg|\geq 1)\}
