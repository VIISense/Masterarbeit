...
A system of inequalities is, like the name says, a set of inequalities e.g. $y<-x+5$ and $y<2x +1$. The main idea to calculate the solution for a system of inequalities is to intersect the solutions of all inequalities in the set. The solution for those two inequalities can be illustrated in the following:
\begin{figure}[H]
\centering
\begin{tikzpicture}
  \begin{axis}[standard,
    ...
    \addplot[mark=none, color=blue]{2*x+1};
    \addlegendentry{$2x+1$}
    \addplot[mark=none, color=red]{(-1)*x+5};
    \addlegendentry{$-x+5$}
    \addplot [fill=green, opacity=0.3] coordinates{(-1.5,-2) (1.333,3.666) (7,-2) (-1.5,-2)};
  \end{axis}
\end{tikzpicture}
\end{figure}

The green area encompasses the solutions of the systems which is the intersection of the solutions of both inequalities. We can see that for certain $y$-values only a handful $x$-values hold the inequalities. For example if $y=1$ then the inequalities only holds if $x\in ]0,4[$. \\
To adapt this idea onto our problem we have to consider for an assertion $x:succ(k\lesseqgtr l$) all assertions $x:succ(i\lesseqgtr j)$ such that by increasing $l$ the cardinality $i$ or $j$ can also increases. Then we translate the cardinality constraints into a set of inequalities. For our two example above we have:
\begin{figure}[H]
\begin{minipage}[t]{.5\textwidth}
\raggedright
\begin{align*}
...
\begin{tikzpicture}[scale=0.8]
  \begin{axis}[
    ...
    grid={both}]
    \addplot[mark=none, color=blue,line width=2pt]{2*x};
    \addlegendentry{$2x<y$}
    \addplot[mark=none, color=red,line width=2pt]{(1/3)*x};
    \addlegendentry{$3y<x$}
    \addplot[mark=none, color=green,line width=2pt] coordinates{(1,-1)(1,4)};
    \addlegendentry{$1\leq x$}
    \addplot[opacity=0.2, fill=blue] coordinates{(-.5,-1) (2,4) (-1,4) (-1,-1) (-0.5,-1)};
    \addplot[fill=green, opacity=0.2] coordinates{(1,-1) (1,4) (6,4) (6,-1) (1,-1)};
     \addplot[fill=red, opacity=0.2] coordinates{(-1,-0.333)(-1,-1)(6,-1)(6,2)(-1,-0.333)};
  \end{axis}
\end{tikzpicture}
\end{minipage}%
\begin{minipage}[t]{.5\textwidth}
...
\begin{tikzpicture}[scale=0.8]
  \begin{axis}[
    ...
    minor tick length=0pt,
    grid={both}]
    \addplot[mark=none, color=blue,line width=2pt]{0.5*x};
    \addlegendentry{$x\leq 2y$}
    \addplot[mark=none, color=red,line width=2pt]{3*x};
    \addlegendentry{$y\leq 3x$}
    \addplot[mark=none, color=green,line width=2pt] coordinates{(1,-1)(1,4)};
    \addlegendentry{$1\leq x$}
    \addplot[opacity=0.2, fill=blue] coordinates{(-1,-0.5) (6,3) (6,4) (-1,4) (-1,-0.5)};
    \addplot[fill=green, opacity=0.2] coordinates {(1,-1) (1,4) (6,4) (6,-1) (1,-1)};
     \addplot[fill=red, opacity=0.2] coordinates{(-0.333,-1) (-1,-1) (-1,4) (1.333,4) (-0.333,-1)};
     \addplot[pattern=crosshatch dots, opacity=0.2] coordinates{(1,3) (2,6) (1,4) (1,3)};
  \end{axis}
\end{tikzpicture}
\end{minipage}
\end{figure}
The left figure we see no area which is contains the solutions for all three inequalities, which means the system has no solution hence for our ABox it means that it is unsatisfiable. The right figure illustrate with the cross-hatched area the intersection of all three solution which denotes that the system has a solution. However not every solution of the system concludes a satisfiable ABox! Since we can only have natural numbers of variables, solutions which only contains only negative numbers and/or only rational numbers which can not be reduced to natural numbers also infer unsatisfiability of the ABox.\\
For the transforming we want to accumulate all assertions to $S(l)$ first which are depending to each other.\\
\textcolor{red}{algorithm for creating $S(l$) and transforming abox assertion into inequalities}\\
We can gain the solution from a solver (black box). We then have to look up whether there is a solution with only natural numbers. If there is one then we do not block anything. Otherwise:
