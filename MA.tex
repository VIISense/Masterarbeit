...
\end{mydef}
Also like in \cite{1} and \cite{6} we have to be \textit{safe} when introducing new variables otherwise we may end in a endless loop or with a false output. In case of $x:succ(n\,dvd\,l)$ we do not have to consider any special cases because in worst case we have to add $n$-times successors, which counts in $l^{\mathcal{I}(S)_x}$. For any other cardinality constraints 
\begin{mydef}[Safe]
Let $x:succ(k\lesseqgtr l)$ be an assertion in $S$ where $k=n_1\cdot|s_1|+\dots+n_i\cdot|s_i|$ and $l=m_1\cdot|t_1|+\dots+m_i\cdot|t_i|$, $i,j\in\mathbb{N}$. It is \textit{safe} to introduce a new variable $y$ and add $(x,y):t$, $t=t_1\cap \dots \cap t_i$ with $i\leq m$, to $S$ if $n_k(t)<n_l(t)$. The numbers $n_k(t)$ and $n_l(t)$ are computed as follows:
\begin{align*}
&n_k(t):=0\\
&\text{ For every }n^\prime\cdot|s^\prime_1\cup\dots\cup s^\prime_p|,\,p\in\mathbb{N},\,\text{in}\, k\\
&\quad\quad\quad\text{if }\exists q:\,1\leq q\leq p\text{ and }s_q\text{ appears syntactically with the same sign in } t \\
&\quad\quad\quad\text{then } n_k(t):=n_k(t)+n^\prime 
\end{align*}
\end{mydef}
