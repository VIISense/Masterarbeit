...
\end{itemize} 
\end{mydef}
Let $\mathcal{I}_1$ and $\mathcal{I}_2$ be two interpretation and $h:\Delta^{\mathcal{I}_1}\mapsto\Delta^{\mathcal{I}_2}$ be a function. $h$ is a homomorphism iff
\begin{itemize}
\item $h(a^{\mathcal{I}_1})=a^{\mathcal{I}_2}$, for all $a\in\Delta^{\mathcal{I}_1}$
\item $a\in C^{\mathcal{I}_1}$ implies $h(a)\in C^{\mathcal{I}_2}$, for all $C\in \mathbf{C}$
\item $(a,b)\in r^{\mathcal{I}_1}$ implies $(h(a),h(b))\in r^{\mathcal{I}_2}$, for all $r\in\mathbf{R}$
\end{itemize}
...
In \cite{knapsack} the calculated upper bound for each $x_i$ is $n\cdot (m\cdot max_{i,j}\{|a_{ij}|,|b_i|\})^{2\cdot m+1}$, where $A$ is a $m\,\times\,n$ matrix, $b$ a vector of $\mathbb{Z}^m$  and $1\leq i\leq m$, $1\leq j\leq n$. In our example the upper bound for all $x_i$ is $5\cdot (2\cdot max_{i,j}\{|1|,|-1|\})^{2\cdot 2+1}=320$. For our example it means that we can have solutions in which we never have more then $320$ successors in each $succ(|A|\leq |B\cap r|)$, $A$ and $B$. 
...
If a $QFBAPA$ solver returns \textit{satisfiable} then select one solution $\phi$ and for each $e\in E(\phi)$ in the solution we introduce a new variable $y$ and
\begin{itemize}
...
\item we apply the $succ$-rule on $x:succ(c)$: By the induction hypothesis we have $x^\mathcal{I}\in succ(c)^\mathcal{I}$. We have to show that by this step we are able to add successors, such that $\mathcal{I}$ satisfies $\mathcal{A}_{i+1}$. In this step, we gather first all $succ$-assertion together, formulate a $QFBAPA$ formula and let a solver return us all possible solution with in an upper bound. Because $\mathcal{A}_i$ is satisfiable, the $succ$-assertions together are also satisfiable (subset of $\mathcal{A}$). Hence there has to solutions which can be returned by the solver. By the induction hypothesis we know that $x^\mathcal{I}\in succ(c)^\mathcal{I}$. In case $x$ has no successor, the solution $\phi$ with $E(\phi)=\emptyset$ must be a valid solution, which can be returned from the solver. In the case where $E(\phi)=\emptyset$ can not be a solution then we have to show that the solver can return a solution, such that there is a homomorphism from $\mathcal{I}(\mathcal{A}_{i+1})$, the (finite) induced interpretation of the given solution, to $\mathcal{I}$, which can be infinite. \textcolor{red}{todo}\\
Therefore if we pick this solution, we can add successors such that $\mathcal{I}$ satisfies $\mathcal{A}_{i+1}$.
\end{itemize}
\end{proof}
