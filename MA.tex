...
$x:succ(k\leq l$) or $x:succ(k\leq l$) and $(x,y):k^\prime$ with $k^\prime$ in $k$ but $(x,y):s\not\in S$ for some $|s|$ in $k$\\
...
\item\label{c}$cardinality$-rule: $S$ contains $x:succ(c)$, with $c\in\{k\leq l,\,k<l,\, n\, dvd\,l\}$, such that $c$ is violated in $\mathcal{I}(S)$ regarding $x$
\begin{enumerate}
...
\item \label{exceeded} if $l\in \mathbb{N}$ does not contain a set term and we have two successor $y_1\neq y_2$ of $x$ such that for a $|s|$ in $k$ we have $(x,y_1):s\in S$ and $(x,y_2):s\in S$ and it is safe to merge them:\\
