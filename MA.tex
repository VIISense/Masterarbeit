...
Let $y_1$ and $y_2$ be successors of a variable $x$. If $y_1$ and $y_2$ are merged into $y$ because of an assertion $x:succ(k\leq l)$ then $b(S,x):=b(S,x)\cup\{s_i|k=n_0+n_1\cdot|s_1|+\dots+n_j\cdot|n_j|,\,\forall i:1\leq i\leq j\}$.
\end{mydef}
...
The last examples always compared a cardinality term with a fix number. However our DL is also capable of comparing two cardinality terms as in $k\leq l$, where $k$ and $l$ can vary during the algorithm.\\
Intuitively for a assertion $x:succ(k\leq l)$ we want to add variables, such that $l$ increases. However if both $k$ and $l$ can increase by a new variable, we have to avoid cases where $k$ increases faster then $l$. Another more crucial problem is when by adding successors not only $k^{\mathcal{I}(S)_x}$ and $l^{\mathcal{I}(S)_x}$ can increases, but also for some other assertion $x:succ(i\leq j)$ the cardinalities $i^{\mathcal{I}(S)_x}$ or $j^{\mathcal{I}(S)_x}$. For that we proceed as follows:
...
At the end we grab again the idea that for an assertion $x:succ(k\leq l)$ we want to increases $l$ and want to avoid that $k$ increases faster then $l$. Hence we introduce the definition of \textit{safe}.
\begin{mydef}[Safe]
Let $x:succ(k\leq l)$ be an assertion in $S$. Let $u=t_1\cap\dots \cap t_j$. If $n_k(u)<n_l(u)$ and $u$ is not blocked by $x$ then $u$ is called \textit{safe regarding $k\leq l$}. The number $n_k(u)$ (and $n_l(u)$ respectively) is computed as followed:
...
First we do the prearrangement: transforming the ABox into the desired form and determining whether some set of assertions $\mathcal{A}$ is satisfiable or not and if needed fill the blocking sets for each variable in $Var(S)$.\\
...
\begin{itemize}
\item $x:succ(k\leq l)$, which is violated
\item there is a set term $s:=|s_1\cap \dots \cap s_i|$, $l=n_1\cdot|s_1|+\dots+n_i\cdot|s_i|+\dots+n_j\cdot|s_j|$, which is safe regarding $k\leq l$
\end{itemize}
...
\item $y$ is successor of $x$
\item $y:s_1$, $\dots$, $y:s_i$, $1\leq i<j$
...
\item the third component is a multiset of integers containing for each $x:succ(k\leq n)\in S$ the number of all successors $y$ of $x$ on which rule \ref{choose} is applicable
\item the fourth component is a multiset which denotes for every $x:succ(k\leq l)$ and $x:succ(n\,dvd\,l)$ the integer $k^{\mathcal{I}(S)_x}\unlhd l^{\mathcal{I}(S)_x}$ and $n\unlhd l^{\mathcal{I}(S)_x}$ respectively
\item the fifth component denotes the number of all successors of $x$.
\item the sixth component is the number of $x$'s successors on which rule \ref{chooserole} is applicable
...
Consider now a tuple $\psi_S(y)$ such that $x\neq y$. $\psi_S(y)$ can only be affected if $x$ is a successor of $y$. The first and second component of $\psi_S(y)$ remain unaffected because both are independent from $x$. The third and fourth component of can never in creases because we do not add any new successors. If at all the third component can decreases because it may happen that by adding assertions regarding $x$ rule \ref{choose} can not be applied any more on some assertion $y:succ(k\leq l)$. The fifth component does not change because we do not add any new successor. The same goes for the fifth component: If there is an assertion $y:succ(k\leq l)$ and by adding $x:C$ or $x:D$ to $S$ neither $k^{\mathcal{I}(S^\prime)_y}$ nor $l^\mathcal{I}(S)_y$ change because by the definition of $\mathcal{I}(S)$ we have $x\in (C\sqcap D)^{\mathcal{I}(S)_y}=C^{\mathcal{I}(S)_y}\sqcap D^{\mathcal{I}(S)_y}\subseteq C^{\mathcal{I}(S)_y}$ (or $\subseteq D^{\mathcal{I}(S)_y}$). This means that $x$ is not added to any further extension in $\mathcal{I}(S^\prime)$ and hence $k^{\mathcal{I}(S^\prime)_y}$ nor $l^\mathcal{I}(S)_y$ remain the same. The sixth component also does not increase: Either rule \ref{chooserole} is applicable on $x$ from the beginning but then it remains that way because we do not add any asertion of the form $(y,x):r,r\in\mathbf{R}$. If the rule was not applicable then it remains that way because there must be an assertion $(y,x):r,\,r\in\mathbf{R}$, in $S$ which is not going to be deleted. The seventh component also do not change: From the beginning we have $x:C\sqcap D$, which means that if rule \ref{leq} is not applicable then it stays inapplicable after $x:C$ or $y:D$ is added. We can conclude that the tuple $\psi_S(y)$ remains in total unchanged or decreases.\\
