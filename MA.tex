...
Note that in a assertion in $NNF$ the negation sign can only occurs in front of a concept name and above a role name.
...
\end{itemize}
Similar in \cite{1} and \cite{6}, where a variable can be replaced by another variable, we will have to merge two variables during the Tableau-algorithm.
\begin{mydef}[Merge]
...
Dealing with $\mathcal{ALCSCC}$ concept is challenging, especially cardinality constraints. Therefore we restrict cardinality constraint to a more simple form.
\begin{mydef}[$CNNF$]
...
\end{mydef}
We want to split $|s_1\cup s_2\cup\dots \cup s_n|$ into set terms such that they are disjoint to each other. This helps to determinate all properties of successors in $k$.\\
We can easily see that the splitting creates exponential new set terms. To simplify them we omit \textit{unnecessary} set terms. To explain what set terms are unnecessary we look at the following example:
\begin{align*}
S:=\{x:succ(|r\cap s\cap t\cap B|<|r|+|s|)\sqcap succ(|r\cap C|=1)\}
\end{align*}
The first cardinality term $|r\cap s\cap t|$ in $CNNF$ is split into seven cardinality terms. However $t$ is not mentioned in any other assertions of $x$. That means that whether $x$'s successor is a $t$-successor or not it does not bring any further information regarding $x$ and the successor. Also no assertions can become violated solely by this information. (Note that this algorithm works without a TBox in which axioms regarding $t$-successor may occur and therefore $t$ would be not unnecessary.) For the same reason the concept name $B$ is also unnecessary. On the other hand $r$ is necessary because on the right hand side of the cardinality constraint it appears again. For the same reason $s$ is necessary, too. Also the $r$-successor can be in $C$ which is important for the second concept. 
\begin{mydef}(Simplified $CNNF$)
Let $x:succ(c)$ be in $S$ and $c$ be a cardinality constraint. A set term $s$ in $c$ is \textit{unnecessary} and can be omitted if each role name $r$ and each concept name $C$ in $s$ does not appear in any IDK
\end{mydef}
...
Let $x:succ(k\leq l)$ or $x:succ(k<l)$, where $l$ has at least one cardinality term $|s|$, be violated regarding $x$ in an interpretation $\mathcal{I}$. It is \textit{safe} to introduce a new variable $y$ and add $(x,y):l$ to $S^\prime:=S\cup\{(x,y):l\}$ if 
\begin{align*}
...
\begin{mydef}[Tableau]
Let $S$ be a set of assertions in $CNNF$.
\begin{enumerate}
...
\item\label{setterm}$cardinality_1$-rule: $S$ contains $x:succ(c)$, with $c\in\{k\leq l,\,k<l,\,n\,dvd\,l\}$, such that $c$ is violated in $\mathcal{I}(S)$ regarding $x$. Choose either $i:=k$ or $i:=l$, such that $i$ contains at least one cardinality term $|s|$. If it is safe to add a variable $y$ in $i$ then\\
$\rightarrow$ choose a set term $s$, such that $|s|$ is in $i$, and $S:=S\cup\{(x,y):s\}$, then jump to rule \ref{repeat}
\item\label{exceeded}$cardinality_2$-rule: $S$ contains $x:succ(c)$, with $c\in\{k\leq l,\,k<l\}$, which is violated in $\mathcal{I}(S)$ regarding $x$, and $l$ does not contain a set term. If we have two successor $y_1\neq y_2$ of $x$ such that for a set term $s$ in $k$ we have $(x,y_1):s\in S$ and $(x,y_2):s\in S$ and by merging them no satisfied constraint regarding $x$ becomes violated, then:\\
...
A \textit{derived set} is a constraint set $S^\prime$ where rule \ref{repeat} is not applicable.
\end{mydef}
...
&S=\{x:succ(|r^\neg|\geq 1)\}
\end{align*} 
It states that $x$ have at least one successor which is not a $r$-successor. Since $\mathbf{R}$ only contains $r$ and $s$ we know that the successors must be $s$-successors. First we apply rule \ref{setterm} to actually add a successor. Therefore $y$ is introduced (it is safe because $n_{\mathcal{I}(S)}(x,r^\neg,S)$ increases) and $(x,y):r^\neg$ is added to $S$. Now no more rules are applicable except for rule \ref{chooserole}. With that rule we can pick either $r$ or $s$. We can not pick $r$ because $r^\neg$ occurs in the assertion. Therefore we have to pick $s$.
\end{ex}
...
\end{align*}
\end{ex}
Note that $|s\cap r|+|s\cap r^\neg|+|s^\neg\cap r|=|s\cup r|$. No matter how often we add a successor in $|r|$ or $|s|$, the left hand side always increases and therefore this assertion stays violated. However if we add a successor in $|s\cap r|$ the assertion becomes satisfied. Hence for this example we can choose rule \ref{setterm} and then choose $i=|s\cap r|+|s\cap r^\neg|+|s^\neg\cap r|$. We then introduce $y$ and add $(x,y): s\cap r$ to $S$. Also note that we can not choose $i=|s|+|r|$ because it is not safe: By adding $(x,y):s$ or $(x,y):r$ to $S$ the number on the right hand side of the cardinality term does not increases faster then the left hand side.\\
We consider now two examples to explain rule \ref{exceeded}
\begin{ex}
...
\}
\end{align*}
By applying rule \ref{setterm} and add a new variable $y$ we satisfies at least one assertion from the second line but still at least one assertion remains violated. If we apply it a second time to satisfy the remaining assertion(s) then $x:succ(|r|=1)$, which was satisfied before, becomes violated. In case we decided to apply \ref{setterm} on $x:succ(|r|=1)$ and $x:succ(|r\cap s|=1)$ we have the option of either applying rule \ref{setterm} on $x:succ(|r\cap C|=1)$ or applying rule \ref{exceeded} on the two new variables. We decide for the first choice:
\begin{figure}[H]
\centering
\begin{tikzpicture}
\node (0) at (0,0) {$x$};
...
\end{align*}
First we apply the rules \ref{setterm} two times to add two $r$-successors for $x$ to satisfy $x:succ(|r|\geq 2)$. But this results to $x:succ(|r|<2)$ being violated. If we ignore the condition in rule \ref{exceeded} and apply it then we merge the two successors leading to $x:succ(|r|<2)$ being satisfied but $x:succ(|r|\geq 2)$ being violated. Then we can apply the rule \ref{setterm} again leading to $x:succ(|r|<2)$ being violated and so on. By the condition in \ref{exceeded} we can not merge the two new variables because the satisfied assertion $x:succ(|r|\geq 2)$ would become violated. Hence the algorithm terminates with a clash stating that the constraint set is unsatisfiable.
\end{ex}
...
\item For every interpretation $\mathcal{I}$ of a satisfiable constraint set $S$ we can find a chain of rule application such that the induced interpretation $\mathcal{I}(S)$ is equal to $\mathcal{I}$
\end{itemize}
