...
\end{align*}
The first cardinality term $|r\cap s\cap t|$ in $CNNF$ is split into seven cardinality terms. The set term $t$ is not mentioned in any other cardinality terms regarding $x$. That means that whether $x$'s successor is a $t$-successor or not it does not bring any further information regarding $x$ and the successor. Also no assertions can become violated solely by this information. (Note that this algorithm works without a TBox in which axioms regarding $t$-successor may occur and therefore $t$ would be not unnecessary.) For the same reason the concept names $B$ and $C$ is also unnecessary. On the other hand $r$ is necessary because it appears again in the cardinality term $|r|+|s|$, which describe successors of $x$. For the same reason $s$ is necessary, too.\\
\textcolor{red}{still not sure about that...}
...
\end{align*}
First by the $\sqcap$-rule we split the whole constraint into three assertion and add them to $S$.\\
