...
Numerical part:
\begin{itemize}
\item\label{succ} $successor$-rule: $\mathcal{A}$ contains for an unmarked variable $x$ at least one assertion of the form $x:succ(c)$:
\begin{itemize}
...
\item $succ$-rule was applied to create $\mathcal{A}$: Since this rule was applicable neither the $\sqcap$-rule nor the $\sqcup$-rule was applicable.
\end{enumerate}
\end{proof}
\begin{mylem}
